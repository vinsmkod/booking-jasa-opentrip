...
class BookingController extends BaseController
{
    ...
    public function create()
    {
        ...
        if (!$schedule_id) return redirect()->to('/');
        ...
        if (!$schedule) return redirect()->to('/');

        $meetingPoints = $this->meetingPointModel->findAll();
    ...
    public function store()
    {
        ...
        $meeting_point_id  = $this->request->getPost('meeting_point');

        if (!$schedule_id || $participantCount <= 0) {
            return redirect()->back()->with('error', 'Jumlah peserta tidak valid');
        }

        $schedule = $this->scheduleModel->find($schedule_id);
        if (!$schedule) return redirect()->back()->with('error', 'Jadwal tidak ditemukan');
        if ($schedule['available'] < $participantCount) return redirect()->back()->with('error', 'Kuota tidak cukup');

        $trip = $this->tripModel->find($schedule['trip_id']);
        if (!$trip) return redirect()->back()->with('error', 'Trip tidak ditemukan');
    ...
    // ==============================
    // KONFIRMASI PEMBAYARAN (ADMIN/USER)
    // ==============================
    public function confirm($bookingId)
    {
        $payment = $this->paymentModel->where('booking_id', $bookingId)->first();
        if (!$payment) return redirect()->back()->with('error', 'Pembayaran tidak ditemukan');
    ...
    // ==============================
    // EXPORT BOOKING KE EXCEL (INCLUDE MEETING POINT)
    // ==============================
